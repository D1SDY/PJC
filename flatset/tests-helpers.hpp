...
template <typename T, typename = std::enable_if_t<is_flat_set<T>::value>>
...
bool operator<(const ordered& o1, const ordered& o2);
std::ostream& operator<<(std::ostream &os, const ordered &o);
...
class scope_guard {
public:
    // This is maybe the only use of function try block.
    template <typename F>
    scope_guard(F &&func)
        try
